...
import { ZoomIn, ZoomOut, LineChart, BarChart2, Pencil, Undo2, Trash2 } from 'lucide-react';
...
export default function Chart({
  ...
  // States for user drawing/annotations feature
  const [drawings, setDrawings] = useState<ChartDrawing[]>([]);
  const [isDrawingMode, setIsDrawingMode] = useState(false);
  const [drawingStart, setDrawingStart] = useState<{ x: number; y: number } | null>(null);
  const [currentDrawLine, setCurrentDrawLine] = useState<{ startX: number; startY: number; endX: number; endY: number } | null>(null);
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    // Handle live update of active drawings drag
    if (isDrawingMode && drawingStart && currentDrawLine) {
      setCurrentDrawLine({
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    setDrawingStart({ x, y });
    setCurrentDrawLine({ startX: x, startY: y, endX: x, endY: y });
  };
  ...
  const handleMouseUp = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    // Map starting and ending x, y coordinates to historical time-stamps and prices
    const sliceCount = Math.min(ticks.length, zoomLevel);
    ...
    const mainChartHeight = showRSI ? dimensions.height * 0.77 : dimensions.height;

    const getPriceFromY = (yVal: number) => {
      const ratio = (mainChartHeight - 20 - yVal) / (mainChartHeight - 40);
      return adjustedMin + ratio * adjustedPriceRange;
    };

    const getTimeFromX = (xVal: number) => {
      const activeWidth = dimensions.width - 75;
      const pct = (xVal - 10) / activeWidth;
      const visibleIdx = Math.round(pct * (sliceCount - 1));
      const clampedIdx = Math.max(0, Math.min(sliceCount - 1, visibleIdx));
      return visibleTicks[clampedIdx]?.time || Date.now();
    };

    const startTime = getTimeFromX(drawingStart.x);
    const startPrice = getPriceFromY(drawingStart.y);
    const endTime = getTimeFromX(x);
    const endPrice = getPriceFromY(y);

    const newDrawing: ChartDrawing = {
      id: `draw-${Math.random().toString(36).substring(2, 10)}`,
      startTime,
      startPrice,
      endTime,
      endPrice,
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Vertical time lines (grid)
      const timeGridCount = 6;
      for (let i = 0; i < timeGridCount; i++) {
        const xPos = 10 + (i * (width - 85)) / (timeGridCount - 1);
        ctx.beginPath();
        ctx.moveTo(xPos, 0);
        ctx.lineTo(xPos, mainChartHeight);
        ctx.stroke();
      }

      // Horizontal pricing lines (grid)
      ...
      if (localChartType === 'line') {
        ...
        if (visibleCandles.length > 0) {
          ...
          visibleCandles.forEach((candle, idx) => {
            ...
            // Standard Forex Candle Colors (Teal for Bull, Red for Bear)
            ...
            // Ensure minimum 1px height for doji candles
            const bodyH = Math.max(Math.abs(openY - closeY), 1.0);
            
            // Solid filled body for both bullish and bearish candles
      ...
      // --- DRAW USER DRAWINGS & ANNOTATIONS ---
      if (drawings && drawings.length > 0) {
        ctx.lineWidth = 2.5;
        drawings.forEach((draw) => {
          // Find matching X offset for start point
          let sx = 0;
          const startIdx = visibleTicks.findIndex(t => t.time >= draw.startTime);
          if (startIdx !== -1) {
            sx = getX(startIdx, visibleTicks.length);
          } else {
            // Out of bounds - extrapolate
            if (draw.startTime < visibleTicks[0].time) {
              sx = getX(0, visibleTicks.length) - 50;
            } else {
              sx = getX(visibleTicks.length - 1, visibleTicks.length) + 50;
            }
          }

          // Find matching X offset for end point
          let ex = 0;
          const endIdx = visibleTicks.findIndex(t => t.time >= draw.endTime);
          if (endIdx !== -1) {
            ex = getX(endIdx, visibleTicks.length);
          } else {
            // Out of bounds - extrapolate
            if (draw.endTime < visibleTicks[0].time) {
              ex = getX(0, visibleTicks.length) - 50;
            } else {
              ex = getX(visibleTicks.length - 1, visibleTicks.length) + 50;
            }
          }

          const sy = getY(draw.startPrice);
          const ey = getY(draw.endPrice);

          // Render connecting trendline
          ...
          // Render beautiful structural endpoints handles
      ...
      // Draw active temporary line preview
      if (currentDrawLine && drawingStart) {
        ctx.strokeStyle = 'rgba(245, 158, 11, 0.8)';
        ctx.lineWidth = 2;
        ctx.setLineDash([4, 4]);
        ctx.beginPath();
        ctx.moveTo(currentDrawLine.startX, currentDrawLine.startY);
        ctx.lineTo(currentDrawLine.endX, currentDrawLine.endY);
        ctx.stroke();
        ctx.setLineDash([]);
      }
      ...
      if (hoveredIndex !== null && mousePos !== null) {
        ...
        if (visibleIndex >= 0 && visibleIndex < visibleTicks.length) {
          ...
          // Subtle pulsing thickness on crosshair based on timestamp
          const pulseSpeedFactor = Date.now() / 180;
          const crosshairPulse = 1.0 + Math.sin(pulseSpeedFactor) * 0.15;

          ctx.strokeStyle = isDark ? 'rgba(255, 255, 255, 0.28)' : 'rgba(15, 23, 42, 0.18)';
          ctx.lineWidth = 1 * crosshairPulse;
          ctx.setLineDash([5, 5]);
          ...
          // Draw a magnificent outer pulsing glow halo
          const dotRadiusPulse = 4.5 + Math.sin(pulseSpeedFactor) * 1.5; // continuous smooth scale animation
          ctx.beginPath();
          ctx.arc(x, y, dotRadiusPulse * 2.2, 0, Math.PI * 2);
          ctx.strokeStyle = isDark ? 'rgba(168, 85, 247, 0.20)' : 'rgba(168, 85, 247, 0.14)';
          ctx.lineWidth = 1.5;
          ctx.stroke();

          // Draw middle halo ring
          ctx.beginPath();
          ctx.arc(x, y, dotRadiusPulse * 1.4, 0, Math.PI * 2);
          ctx.strokeStyle = isDark ? 'rgba(168, 85, 247, 0.35)' : 'rgba(168, 85, 247, 0.22)';
          ctx.lineWidth = 1.5;
          ctx.stroke();

          // Draw main interactive dot on the price path
          ctx.beginPath();
          ctx.arc(x, y, dotRadiusPulse, 0, Math.PI * 2);
    ...
    <div className={`relative flex flex-col rounded-xl border p-4 shadow-sm w-full h-[360px] sm:h-[420px] transition-colors ${
      ...
      <div className={`flex flex-wrap items-center justify-between border-b pb-3 mb-3 gap-2 ${
        ...
        <div className="flex items-center space-x-2">
          ...
          {/* Drawing Tools (Pencil drawing, Undo, Clear) */}
          <div className={`flex space-x-0.5 rounded-lg p-0.5 border ${
            ...
            <button
              ...
              className={`rounded p-1 transition-all cursor-pointer ${
                isDrawingMode
                  ? isDark ? 'bg-amber-500 text-slate-950 font-black' : 'bg-amber-500 text-slate-950 font-black'
                  : isDark ? 'text-slate-400 hover:text-white hover:bg-slate-900/40' : 'text-gray-400 hover:text-black'
              }`}
              title="Draw Trendline (Click & Drag)"
            >
            ...
            {drawings.length > 0 && (
              <button
                onClick={() => setDrawings([])}
                className={`rounded p-1 transition-all cursor-pointer ${
                  isDark ? 'text-rose-500 hover:text-rose-400 hover:bg-slate-900/40' : 'text-rose-500 hover:text-rose-600'
                }`}
